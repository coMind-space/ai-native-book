...
Архитектура корпоративного ассистента управляет доверием: кто имеет доступ к данным, что может делать ассистент, где проходит граница безопасности, как проверяется результат, что сохраняется в памяти компании и где человек принимает решение.
...
> Агент не должен иметь прямой хаотичный доступ к корпоративным системам. Ему нужен управляемый слой между человеком, моделью и данными.
...
Название конкретного инструмента вторично. Все такие инструменты должны подчиняться общим правилам:
...
В корпоративной архитектуре особенно важна роль шлюза.
...
Главный принцип работы шлюза: ассистент действует только в разрешенных границах.
...
Прямые токены трудно контролировать, отзывать, ограничивать, проверять и безопасно хранить. Кроме того, агент начинает жить как отдельный технический пользователь в отрыве от конкретного человека и его роли.

В зрелой модели агент не владеет корпоративными секретами. Он обращается к инструментам через шлюз.

## Права доступа

Права должны учитывать контекст.
...
Описание прав должно учитывать, какие данные человек видит и какие действия поручает ассистенту.
...
- **Короткая память:** текущая задача, текущая сессия, загруженные документы, уточнения пользователя и временные решения.
- **Средняя память:** контекст проекта, профиль клиента, повторяющиеся предпочтения команды, резюме прошлых встреч и факты, полезные для работы роли.
- **Длинная память:** знания, подтвержденные источниками, решения, шаблоны, регламенты, архитектурные записи и проверенные выводы.
