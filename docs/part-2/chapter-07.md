...
Если ассистент не знает, где лежит правда, он начинает опираться на то, что доступно: общий контекст модели, старые документы, неполные заметки, случайные фрагменты переписки, предположения пользователя или собственные вероятностные догадки.

В обычной личной задаче это может быть терпимо. Человек попросил идею, получил черновик, поправил.
...
Source of truth может быть распределен по нескольким системам.
...
Source of truth — это управленческая дисциплина.
...
Поэтому локальный файл может быть полезен как временная памятка, но не должен становиться source of truth.

Правильная модель: ассистент должен обращаться к живым источникам вместо статичного описания компании.

## Живая карта источников

Компании нужна живая карта источников. Она отвечает на вопрос: где лежит правда по каждому домену компании?

Пример:

- люди и роли — кадровая система, оргструктура, база знаний;
- клиенты и сделки — CRM, коммерческие документы, протоколы встреч;
- проекты — система задач, проектная документация, решения;
- процессы — база знаний, регламенты, схемы процессов;
- документы — корпоративное хранилище, договорной контур, шаблоны;
- решения — протоколы, архитектурные решения, управленческие записи;
- оперативная активность — задачи, календарь, коммуникации, статусы.
...
Так начинается взрослая работа с источниками данных.
...
Плохая модель звучит так:

> Подключим ассистента ко всем документам, чтобы он лучше отвечал.

Хорошая модель устроена иначе: ассистент обращается к source of truth через MCP-шлюз или другой управляемый слой. Сначала проверяются личность пользователя, роль, права на ресурс и тип действия. Только после этого ассистент получает нужные данные.
...
В AI-Native компании главный вопрос звучит так: имеет ли этот человек право получить эти данные через ассистента для этой задачи?
...
Если компания работает с внешними моделями, этот вопрос становится особенно важным.

Правильный принцип обезличивания: слой обезличивания должен находиться до передачи данных в модель.

Иначе получается странная ситуация: сырые персональные или конфиденциальные данные уже ушли в LLM, а потом мы просим ее «обезличить результат».
...
Минимизация данных помогает сохранять доверие.

## Source of truth для типовых документов

Часто первое внедрение ИИ можно начинать с типовых документов, без больших интеграций.
...
Например, для реквизитов клиента:
...
2. согласованный протокол разногласий;
3. актуальный шаблон;
4. комментарии юриста.

Для статуса задачи: 1. система задач;

2. решение на статусной встрече;
3. сообщение ответственного;
4. устное обещание.

Такие правила кажутся мелочью, пока не случается ошибка.

Ассистент должен давать ответ и показывать, когда источники расходятся:

> В источниках есть противоречие. В CRM указан один срок, в протоколе встречи другой. По правилу приоритета главным считаю протокол встречи, но требуется подтверждение владельца проекта.
...
Именно так source of truth становится практическим инструментом, а проект базы знаний получает рабочую основу.
...
Если на часть вопросов нет ответа, ИИ сделал видимым список управленческих долгов.
...
Source of truth — это практическая дисциплина: для каждой роли и каждого skill понимать, где лежит правда и кто за нее отвечает.
