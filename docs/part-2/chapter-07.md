...
Хорошая модель устроена иначе:

> Ассистент обращается к source of truth через MCP-шлюз или другой управляемый слой. Сначала проверяются личность пользователя, роль, права на ресурс и тип действия. Только после этого ассистент получает нужные данные.
