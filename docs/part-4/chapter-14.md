...
Обратная ошибка — не начинать, пока не будет идеальной архитектуры. Нужен MCP-шлюз, единая авторизация, память, права, аудит, закрытый контур, все интеграции, админка, мониторинг, реестр skills, evals, дашборды. Все это действительно нужно для зрелой системы. Но если ждать полного набора до первого рабочего контура, компания может потерять месяцы.
