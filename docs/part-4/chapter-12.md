...
Первый месяц нужен для проверки контура, где есть:
- роль;
...
Первые 30 дней должны доказать, что компания способна превратить ИИ в управляемую работу.
...
Важно, чтобы он мог сказать: «Да, это мой процесс. Я готов проверить новый способ работы и отвечать за внедрение».
...
| --- | --- |
| Повторяемость | Задача возникает достаточно часто, чтобы измерить улучшение |
| Боль | Команда уже теряет время, качество или управляемость |
| Доступность данных | Source of truth хотя бы частично понятен |
| Безопасность старта | Можно начать с Delegate и Review без широкого Act |
| Измеримость эффекта | Есть показатель, который можно сравнить до и после |

Сценарий с высоким суммарным баллом подходит для первой волны.

Но есть еще один важный фильтр: есть ли человек, который будет владельцем этого контура?
...
Формулировка «у нас будет ассистент для отдела» слишком размыта. Усиленную роль нужно описать конкретнее:

- роль: менеджер по продажам;
- контур: подготовка коммерческого предложения;
- цель: сократить время подготовки и повысить качество входных данных;
- ассистент помогает собрать контекст, найти похожие материалы, подготовить структуру, проверить полноту и сформировать черновик;
- человек отвечает за позиционирование, обещания клиенту, финальную редакцию и отправку.

Для PM это может выглядеть так:

- роль: руководитель проекта;
- контур: еженедельный статус;
- ассистент помогает собрать задачи, выделить блокеры, проверить просрочки, подготовить summary и найти открытые решения;
...
У каждого skill должен быть ответ на вопрос: где ассистент берет правду?

Ответ должен вести к конкретным источникам:
- CRM;
- трекер;
- база знаний;
- файл шаблона;
...
Если source of truth нет, это тоже результат. Первый контур покажет пользу ИИ и одновременно выявит пробел в управлении знаниями.
...
Важно: это должны быть реальные рабочие задачи. На учебных примерах команда не почувствует ценности.
...
После первых запусков skills почти всегда нужно переписать. Это нормально. Skill рождается в столкновении с реальной работой.

## Неделя 4. Измерение, разбор и решение о следующей волне

Четвертая неделя нужна для управленческого вывода, а не для празднования или поиска виноватых.
...
Первая группа должна быть маленькой, но настоящей. Хороший размер — от 3 до 7 человек.
...
Группа из одних энтузиастов покажет красивые примеры и скроет реальную сложность внедрения. Группа из одних скептиков может остановить пилот до того, как он заработает.
...
Плохое объяснение звучит так: «Теперь у нас ИИ, он будет повышать эффективность».

Такое звучит угрожающе и пусто.

Лучше объяснить так: «Мы берем один повторяющийся процесс и пробуем убрать из него ручную рутину. Ассистент будет помогать готовить черновики, искать данные и проверять качество. Решения остаются за людьми. Через месяц мы посмотрим, стало ли быстрее и лучше».

Еще лучше, если руководитель прямо скажет: «Наша цель — собрать новый способ работы. Ошибки ассистента и неудобства процесса нужно фиксировать, потому что именно так мы улучшаем skills».
...
Часто сопротивление связано с неопределенностью: что теперь ждут от людей, как будет оцениваться работа, кто отвечает за ошибку и не станет ли ассистент способом контроля.
